                            ...
                            <div className="flex-1 space-y-3">
                                ...
                                {(group?.rows || []).map((row: any, idx: number) => {
                                    ...
                                    return (
                                        <motion.div
                                            key={idx}
                                            className={`grid grid-cols-[25px_1fr_30px_30px_30px] items-center text-xs border rounded-xl px-2 py-2.5 transition-colors ${isAdvancing ? 'bg-orange-50/50 border-orange-100' : 'bg-slate-50/30 border-slate-50'}`}
                                            whileHover={reduced ? undefined : { scale: 1.01, backgroundColor: isAdvancing ? 'rgba(255,237,213,0.8)' : 'rgba(248,250,252,0.9)' }}
                                        >
